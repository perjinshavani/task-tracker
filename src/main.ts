...
// Valfri uppgift:
// Detta interface ärver alla egenskaper från Task
// och lägger till metoden toggle() som kan växla
// status mellan Pending och Completed.

interface TaskWithToggle extends Task {
toggle(): void;                                     
}
...
function addTask(name: string, priority: Priority): void {
  const newTask: Task = {
    id: nextId,
    name: name,
    status: Status.Pending,
...
//Den loopar igenom alla tasks i arrayen.
  for (const task of tasks) {
    ...
    description.textContent = `Beskrivning: ${
      task.description ?? "Ingen beskrivning"
    }`;

    const notes = document.createElement("p");
    notes.textContent = `Anteckningar: ${
      task.notes ?? "Inga anteckningar"
    }`;
